...
int main(int argc, char** argv) {
    ...
    memcpy(ap_ssid, argv[1], sizeof(ap_ssid));
    ap_passwd = (char*)malloc(sizeof(argv[2]));
    memcpy(ap_passwd, argv[2], sizeof(ap_passwd));
    mac = (char*)malloc(sizeof(argv[3]));
    memcpy(mac, argv[3], sizeof(mac));
    ip = (char*)malloc(sizeof(argv[4]));
    memcpy(ip, argv[4], sizeof(ip));
    ...
    {
       sprintf(buf, "%d%s%s%d", ssid_pass_length, mac, ip, total_length); //产生：" 123 4567"
